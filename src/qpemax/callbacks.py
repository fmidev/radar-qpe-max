...
class ProgressLogging(Callback):
    """Log dask computation progress and memory usage.

    Works under both local (synchronous/threaded) and distributed schedulers.
    Local schedulers get task-level progress via Callback hooks; under
    distributed only elapsed time and memory are reported.
    """
    ...
    def _update(self, elapsed):
        ...
        if s is not None:
            ...
            self._logger.info(
                f"running for {format_time(elapsed)}, mem: {mem:.1f} MB"
            )
